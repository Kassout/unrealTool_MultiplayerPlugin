	...
	JoinSessionCompleteDelegate(FOnJoinSessionCompleteDelegate::CreateUObject(this, &ThisClass::OnJoinSessionComplete))
{
	...
	// Connect player character to online subsystem.
	IOnlineSubsystem* OnlineSubsystem = IOnlineSubsystem::Get();
	// Check online subsystem validity.
	if (OnlineSubsystem)
	{
		// Use online subsystem to create a session interface.
...
{
	// Called when pressing the Numpad 1 key.
	...
	// Add function delegate for an on create session complete event.
	OnlineSessionInterface->AddOnCreateSessionCompleteDelegate_Handle(CreateSessionCompleteDelegate);

	// Setup online session settings.
...
void AMenuSystemCharacter::JoinGameSession()
{
	...
	// Add function delegate for on find sessions complete event.
	OnlineSessionInterface->AddOnFindSessionsCompleteDelegate_Handle(FindSessionsCompleteDelegate);

	// Setup session search settings.
...
void AMenuSystemCharacter::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
	if (bWasSuccessful)
	{
		...
		// Character travel to "Lobby map" on create session complete.
		UWorld* World = GetWorld();
		if (World)
		{
			// Open "Lobby" map as a listen server.
